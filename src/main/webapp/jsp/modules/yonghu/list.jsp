...
<!--/Pre Loader -->
    ...
    <!-- 本表的查询条件 -->
        ...
        function setDataRow(item, number) {
            ...
            //办理月卡
            var editBtn = document.createElement('button');
            var editAttr = 'xiaofei(' + item.id +','+'1)';
            editBtn.setAttribute("type", "button");
            editBtn.setAttribute("class", "btn btn-warning btn-sm 修改");
            editBtn.setAttribute("onclick", editAttr);
            editBtn.innerHTML = "办理月卡";
            btnGroup.appendChild(editBtn);

            //办理年卡
            var editBtn = document.createElement('button');
            var editAttr = 'xiaofei(' + item.id +','+'2)';
            editBtn.setAttribute("type", "button");
            editBtn.setAttribute("class", "btn btn-warning btn-sm 修改");
            editBtn.setAttribute("onclick", editAttr);
            editBtn.innerHTML = "办理年卡";
            btnGroup.appendChild(editBtn);
        ...
        function xiaofei(id,flag) {
            ...
            var mymessage = confirm("确定要购买"+tempString+"么？");
            if (mymessage == true) {
                http("yonghu/xiaofei", "GET", {
                    id: id,
                    flag: flag,
                }, (res) => {
                    if(res.code == 0) {
                        alert("购买"+tempString+"成功");
                        getDataList();
                    }else{
                        alert(res.msg);
                    }
